...
MeStatus me_op_soft_conv(MeOpContext *ctx) {
    ...
    if (!x || !w || !y) return ME_STATUS_ERROR_INVALID_ARGUMENT;
    if (me_tensor_dtype(x) != ME_SCALAR_FLOAT32 ||
        me_tensor_dtype(w) != ME_SCALAR_FLOAT32 ||
        me_tensor_dtype(y) != ME_SCALAR_FLOAT32)
        return ME_STATUS_ERROR_UNSUPPORTED;

    int32_t xs[4], ws[4], ys[4];
    uint32_t xnd = 4, wnd = 4, ynd = 4;
    if (me_tensor_shape(x, xs, &xnd) != ME_STATUS_OK ||
        me_tensor_shape(w, ws, &wnd) != ME_STATUS_OK ||
    ...
    if (!infer_conv_params(H, KH, OH, &sh, &ph) ||
        !infer_conv_params(W, KW, OW, &sw, &pw))
        return ME_STATUS_ERROR_UNSUPPORTED;
