...
MeStatus me_op_soft_softmax(MeOpContext *ctx) {
    ...
    if (me_tensor_shape(in, NULL, &ndim) != ME_STATUS_OK || ndim == 0)
        return ME_STATUS_ERROR_INVALID_ARGUMENT;
    int32_t shape[8];
    if (ndim > 8) return ME_STATUS_ERROR_UNSUPPORTED;
    uint32_t cap = ndim;
    if (me_tensor_shape(in, shape, &cap) != ME_STATUS_OK) return ME_STATUS_ERROR_INVALID_ARGUMENT;

    size_t inner = (size_t)shape[ndim - 1];
    if (inner == 0) return ME_STATUS_ERROR_INVALID_ARGUMENT;
    size_t outer = 1;
    ...
    for (size_t o = 0; o < outer; ++o) {
        ...
        if (sum == 0.0f) return ME_STATUS_ERROR_EXECUTION_FAILED;
        for (size_t i = 0; i < inner; ++i)
            out_row[i] /= sum;
    }
