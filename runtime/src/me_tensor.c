...
static size_t compute_nbytes(MeScalarType dtype, const int32_t *shape,
                             uint32_t ndim) {
...
MeStatus me_tensor_create(MeRuntime rt, MeScalarType dtype,
    ...
    if (nbytes == 0) return ME_STATUS_ERROR_INVALID_ARGUMENT;
    ...
    if (!t->shape) { me_free(a, t); return ME_STATUS_ERROR_OUT_OF_MEMORY; }
...
void me_tensor_destroy(MeTensor tensor) {
    ...
    if (tensor->owns_data) me_free(a, tensor->data);
...
MeStatus me_tensor_set_data(MeTensor tensor, const void *src, size_t size) {
    if (!tensor || !src) return ME_STATUS_ERROR_INVALID_ARGUMENT;
    if (size != tensor->nbytes) return ME_STATUS_ERROR_INVALID_ARGUMENT;
...
MeStatus me_tensor_shape(MeTensor tensor, int32_t *shape_out,
                         uint32_t *ndim_out) {
    if (!tensor || !ndim_out) return ME_STATUS_ERROR_INVALID_ARGUMENT;
...
MeScalarType me_tensor_dtype(MeTensor tensor) {
    return tensor ? tensor->dtype : ME_SCALAR_UNKNOWN;
}
